...
pval_func <- function(crit, stage, core, dataset ){
  ...
  sql <-sprintf("select count(*)
                from %s 
                where name not in ('rbcL')",core_sel)
...
print(sprintf("pgsn_tot: %.2f, pgtn_tot: %.2f",pgsn_tot,pgtn_tot))
...
#print(pval_func("family","hgt_com","normal","core"))
magic <- pval_func("family","hgt_par","normal","core")
