...
class WebGen
 ...
 def initialize()
  ...
  def gene_blo_runs_index

    gene_blo_runs_index_f = "#{Rails.root}/public/exports/gene_blo_runs/index.html"
    gene_blo_runs_dir = "#{Rails.root}/public/exports/gene_blo_runs"

    out_hdl =  File.open(gene_blo_runs_index_f, 'w')

    out_hdl.puts "<html> \n <body>"
    out_hdl.puts "<script type=\"text/javascript\" src=\"jalview.js\"></script>"
    
    out_hdl.puts '<applet name="JalviewLite"  code="jalview.bin.JalviewLite"
archive="jalviewApplet.jar" width="0" height="0">
<param name="debug" value="true"/>
<param name="showbutton" value="false"/>
</applet>
'
    out_hdl.puts "<h1>Orthologs,</h1>"
    out_hdl.puts "<h2>Gblock corrected alignments </h2>"
    out_hdl.puts "<table>"
    out_hdl.puts " <tr> <th>Gene</th>  <th>Original Length</th> <th>Blocks Length</th> <th>Fasta</th> <th>Clustal</th> <th>Plylip</th></tr>"
    ...
    genes.each {|gn|
      #covert to clustal and phylip
#      sys "jruby fasta2phylip.rb #{gn.name}"
#      sys "jruby fasta2clustal.rb #{gn.name}"
      ...
      gene_blo_runs_fas = "/proc_hom/exports/gene_blo_runs/#{gn.name}.fasta"
      gene_blo_runs_clu = "/proc_hom/exports/gene_blo_runs/#{gn.name}.aln"
      gene_blo_runs_phy = "/proc_hom/exports/gene_blo_runs/#{gn.name}.phylip"

      out_hdl.puts " <tr> <td>#{gn.name}</td>  <td>#{this_gene_blo_run.oa_length}</td>  <td>#{this_gene_blo_run.blocks_length}</td> <td> <a href=\"#{gene_blo_runs_fas}\">Fasta</a> </td> <td> <a href=\"#{gene_blo_runs_clu}\">Clustal</a> </td> <td> <a href=\"#{gene_blo_runs_phy}\">Phylip</a> </td>"
      out_hdl.puts " <td>  <input type=\"button\" name=\"#{gn.name}\" value=\"Jalview\" onClick=\"startJalview(\'/proc_hom/exports/gene_blo_runs/#{gn.name}.fasta\',\'#{gn.name}.fasta\',\'alwvar\')\"/> </td>" 
      out_hdl.puts "</tr>"
