class GeneBloRunsController < ApplicationController
  ...
  def show
    ...
    respond_to do |format|
      ...
      format.fasta { send_file "/root/devel/proc_hom/files/proc/gene_blo_runs/#{@gene_blo_run.gene.name}.fasta", :type => 'text/plain', :disposition => 'inline' }
    end
  end
  ...
  def destroy
    ...
    respond_to do |format|
      format.html { redirect_to :controller => 'gene_blo_runs', :action => 'index'  }
      format.json { head :ok }
    end
  end
end
