...
class WinTask
  ...
  def execute
   ...
   Thread.abort_on_exception = true
    ...
    #calculate beast
    cmd = "java -Djava.library.path=#{exec_d}/lib/ -jar #{exec_d}/local/BEASTv1.7.5/lib/beast.jar -beagle_SSE -beagle_instances 1 -overwrite gene_beast.xml"
    puts cmd
    pid = spawn(cmd, {:out => "gen-beast.log"})
    #check loop
    upd_folder_size() 
    check_folder_size()
    Process.wait(pid)
    ...
    sleep 5 
     ...
     #annotate
    cmd = "#{exec_d}/local/BEASTv1.7.5/bin/treeannotator -heights median -burnin 100 -limit 0.5 -target starting_tree.nwk gene_beast3.trees dated_tree.annot"
    puts cmd
