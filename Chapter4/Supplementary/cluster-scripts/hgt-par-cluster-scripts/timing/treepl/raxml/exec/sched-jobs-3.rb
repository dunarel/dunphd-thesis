#!/home/badescud/local/bin/ruby 
 ...
 class SchedJobs 
  ...
  def launch_jobs
   ...
   #pid = Process.ppid
    ...
    @jobs.each_with_index {|it, idx|
      ...
      align_len = (it["fen_idx_max"]).to_i-(it["fen_idx_min"]).to_i - 1

      #spawn current task
      puts "before spawn_one_task..."
      spwn_tsk = spawn_one_task(fen_d, align_len)
  ...
  def spawn_one_task(fen_d, align_len)
    puts "in spawn_one_task"   
    puts "fen_d: #{fen_d}"

    cmd = "#{self.sdir}/exec/win-task-3.rb --threads #{self.opt_threads} --align-len #{align_len}"
    puts cmd
