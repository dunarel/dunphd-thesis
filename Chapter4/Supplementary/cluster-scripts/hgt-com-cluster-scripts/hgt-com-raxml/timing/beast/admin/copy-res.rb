...
require '../lib/local-config.rb'
 ...
 class CopyRes
  ...
  def copy_fens(recreate=false)
    ...
    work_d =  Pathname.new "#{self.sdir}/results"
    puts "work_d: #{work_d}"
    ...
    all_fens_a.each { |fn|
     ...
     fen_src_d = self.sdir + "results" + fn
     ...
     [["res_loc","res_loc_files"]].each{ |fld, var| 
      fen_dst_d = self.ldir + fld + fn
      ...
      begin
       ...
       # puts "@config_yml: #{@files_yml.inspect}"
       # puts "@config_yml[#{var}]: #{@files_yml[var]}"
      @config_yml[var].each{|fl|
       #puts "fl: #{fl}"
       #copy from folder direct to destination
       ...
       if fen_src_f.exist?
        ...
        FileUtils.cp(fen_src_f, fen_dst_f) 
       end
      } if !@config_yml[var].nil?
    }
