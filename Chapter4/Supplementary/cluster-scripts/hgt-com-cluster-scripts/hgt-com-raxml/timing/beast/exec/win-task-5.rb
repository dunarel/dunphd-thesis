...
class WinTask
  ...
  def execute
   ...
   Thread.abort_on_exception = true
    here =  Pathname.getwd()
    dirname, basename = here.split
    fn = dirname

    puts "dirname: #{dirname}, basename: #{basename}"

    #logcombiner
    cmd = "#{exec_d}/local/BEASTv1.7.5/bin/logcombiner -burnin 2000000 gene_beast1.log gene_beast.log"
    puts cmd
    pid = spawn(cmd, {:out => "combiner.log"})
    #check loop
    upd_folder_size() 
    check_folder_size()
    Process.wait(pid)
    ...
    sleep 5 
     ...
     #annotate
     starting_tree_f = self.sdir + "work" + fn + "starting_tree.nwk"
   
    cmd = "#{exec_d}/local/BEASTv1.7.5/bin/treeannotator -heights median -burnin 0 -limit 0.5 -target #{starting_tree_f} gene_beast.trees dated_tree.annot"
    puts cmd
    pid = spawn(cmd, {:out => "annot.log"})
    #check loop
    upd_folder_size()
    check_folder_size()
    Process.wait(pid)
