  ...
  #===========================================================================
    ...
    if(  ($i == 2) || (($bootstrap eq "no") && ($consensus eq "no"))  ){
        ...
        for(my $j=2;$j<= scalar @hgt_tab;){
			print STDOUT "\n j =  " . $j . " et ligne = " . $hgt_tab[$j];
            if($hgt_tab[$j] =~ /^[0-9]/){
                my $cpt = read_line($hgt_tab[$j++]); #2
				print STDOUT "compteur " . $cpt;
                for(my $k=0;$k<$cpt;$k++){ 
					...
					print STDOUT "affichage " . $criterion_list2;
					print STDOUT "affichage " . $transfer_description;
				...
				# $total_hgt = 0;
				# $total_trivial = 0;
				# for(my $x=2;$x<= scalar @hgt_tab;){
				
					# @tmp_tab = split(" ",$hgt_tab[$j]);
					# if($total_hgt < $tmp_tab[1]) {
						# $total_hgt = $tmp_tab[1];
 ...
 #   print STDOUT "mode=$mode";
    ...
    foreach my $elt( @hgt_pos){
		...
		if(($bootstrap eq "yes")) { #&&($hgt_number_tab{"$elt"} !~ "Trivial")){
			...
			printf(SIM "%s<>%s<>%3.1lf\n",$tmp_sim_tab[0],$tmp_sim_tab[1],$hgt_compteur_tab{$elt}*100/$nbTrees) if ($sim_hgt eq "yes"); 
		}
...
sub print_title{
	print STDOUT "===================================================================================================\n";
	print STDOUT "| HGT-DETECTION V.3.4 (November, 2012) by Alix Boc, Alpha Boubacar Diallo and Vladimir Makarenkov |\n"; 
	print STDOUT "===================================================================================================\n";
}
...
sub print_description{
	print STDOUT	"==========================================================================================================\n";
	print STDOUT	"| Program : HGT Detection 3.4 - November, 2012                                                           |\n";
