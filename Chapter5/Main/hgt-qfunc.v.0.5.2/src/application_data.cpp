...
void ApplicationData::process_cmd_line(int argc, char** argv) {
    ...
    try {
        ...
        // that it contains.
        CmdLine cmd("Q Functions HGT, UQAM, 2013-2015", ' ', "0.5.2");
         ...
         */
        ValueArg<string> msa_fastaArg("", "msa-fasta", "Multiple sequence alignment", false, "msa-fasta.fa", "Input MSA Fasta file", cmd);
        ValueArg<string> q_func_hgts_csvArg("", "q-func-hgts-csv", "Output CSV results file", false, "q-func-hgts.csv", "Output CSV results file", cmd);
		ValueArg<string> gr_seqs_csvArg("", "gr-seqs-csv", "Sequence Groups Membership CSV file", false, "gr-seq.csv", "Sequence Groups Membership CSV file", cmd);
        ...
        ValueArg<int> f_opt_maxArg("", "f_opt_max", "Function to optimize option ", false, 1, "Options are 1-4", cmd);
		...
		q_func_hgts_csv_ = q_func_hgts_csvArg.getValue();
		cout << "q_func_hgts_csv: " << q_func_hgts_csv_ << endl;

        gr_seqs_csv_ = gr_seqs_csvArg.getValue();
        cout << "gr_seqs_csv: " << gr_seqs_csv_ << endl;
